...
$T->Set('id_cliente', $sup['f_cli_cod']);
...
$db->Query("SELECT cj_ref,local as suc FROM p_users p, caja c WHERE NAME = 'Developer' AND LOCAL = c.cj_suc AND c.cj_estado = 'Abierta' ORDER BY c.id DESC LIMIT 1");
if ($db->NumRows() > 0) {
    $db->NextRecord();
    ...
    $puntos_exp_sql  = "SELECT DISTINCT p_cod FROM pdv WHERE p_suc = '$suc'";
    $db->Query($puntos_exp_sql);
    
    $puntos_exp = "";
    while($db->NextRecord()){
        $pex = $db->Record['p_cod'];
        $puntos_exp.="<option>$pex</option>";
    }
    $T->Set('puntos_exp', $puntos_exp);    
