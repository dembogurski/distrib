...
$add = $add."#-  Cant. | Precio |  U.M.  |  Subtotal\n<br>";
...
while( $Q0->NextRecord() ){
	...
	$cant = str_pad(number_format($Q0->Record['d_cant'],2,",","."),8,".");
    
	$precio = str_pad(number_format($Q0->Record['d_precio'],0,",","."),10,".");
    
    $um = str_pad(ucfirst( strtolower( $Q0->Record['d_um'])),5); 
	$subt = $Q0->Record['d_subtotal'];
    $subtotal = str_pad(number_format($subt,0,",",".")  ,13,".",STR_PAD_LEFT); 
		 
    
	$descrip =   "  ".$Q0->Record['d_descrip'];
	
	$add.="$n-  ".$cant."".$precio."".$um."".$subtotal."\n<br>";
	$add.=". ".$descrip."\n<br>";
