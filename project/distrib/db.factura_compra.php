...
$Obj->Add(
    array(
        ...
        F_HELP_ => "Genera una nueva Factura de compra",
        F_TYPE_ => "select_list",
        F_OPTIONS_ => "No,Si",
        F_LENGTH_ => "2",
        F_NODB_ => "1",
...
$Obj->Add(
    array(
        ...
        F_TYPE_ => "text",
        F_MAKE_QUERY_ => "1",
        F_QUERY_ => "'SELECT gen_compra('+c_ref.getVal()+', '+c_empr.getStr()+','+c_prov.getVal()+', '+c_fecha.getStr()+', '+c_fecha_fac.getStr()+',  '+c_factura.getStr()+', '+c_moneda.getStr()+', '+c_cotiz.getVal()+','+c_fn.getVal()+','+c_otros.getVal()+','+c_valor_total.getVal()+','+c_porc_rec.getVal()+','+c_tipo.getStr()+',1)'",
...
$Obj->Add(
    array(
        ...
        F_LINK_ => "db.fact_comp_det",
        F_SEND_ => "c_ref.get()",
        F_NODB_ => "1",
        F_ORD_ => "115",
        C_SHOW_ => "((c_estado.get()=='Abierta'&&operation!=INSERT_) )&&c_valor_total.getVal()>0 ",
        G_SHOW_ => "64",
        G_CHANGE_ => "64"));
...
$Obj->Add(
    array(
        ...
        F_ORD_ => "180",
        C_VIEW_ => "false",
        F_FORMULA_ => "document.getElementById(|{c_compras_det}|).setAttribute(|{hidden}|,|{false}|); document.getElementById(|{hbox_c_compras_det}|).setAttribute(|{height}|,|{280}|);",
        G_SHOW_ => "68",
        G_CHANGE_ => "68"));		
